...
public class Data {
    ...
    */
    ...
    // Nova funkcija za update sedežev
    public synchronized void updateAvailableSeats(String flightNumber, String seats) throws DatabaseException {
        try {
            DataInfo flightRecord = find(flightNumber);
            if (flightRecord != null) {
                flightRecord.getValues()[8] = seats;
                modify(flightRecord);
            } else {
                throw new DatabaseException("Flight not found: " + flightNumber);
            }
        } catch (NumberFormatException e) {
            throw new DatabaseException("Invalid available seats value for flight: " + flightNumber);
        }
    }
